...
OUTPUT="results/k4_output.txt"
TIME_FILE="results/k4_time.txt"
SUMMARY="results/k4_summary.txt"
...
COIN_COUNT=$(grep -c '^kprabhakaran;' "$OUTPUT")
echo $OUTPUT
awk '
/^kprabhakaran;[0-9]+[[:space:]]+[0-9a-f]{64}$/ {
    hash=$2
    zeros=0
...
END {
    print max_zeros > "results/k4_highest_zero.txt"
    print best >> "results/k4_highest_zero.txt"
}
' "$OUTPUT"
...
cat results/k4_highest_zero.txt >> "$SUMMARY"
